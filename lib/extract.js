...
function Extract(opts) {
  ...
  // CVE-2018-20834 fix: Intercept "entry" events before fstream.Writer processes them
  // Remove hardlinks synchronously when File entries are about to be processed
  // We need to do this BEFORE fstream.Writer's "entry" listener runs
  var originalEmit = me.emit
  me.emit = function (event, entry) {
    if (event === "entry" && entry && entry.type === "File") {
      // Remove hardlink synchronously before event propagates to fstream
      try {
    ...
    // Call original emit to propagate event (including to fstream.Writer)
    return originalEmit.apply(me, arguments)
  }
